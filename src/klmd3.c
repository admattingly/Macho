/*
 * KLMD3 - COMPUTE LAST MESSAGE DIGEST with CC=3 loop and static buffers
 */
...
#pragma export(KLMD3)
#else
#pragma prolog(KLMD3, main)
#pragma epilog(KLMD3, main)
#endif
        ...
        MACHO_INTEGER   pticks)        /* (output)       elapsed clock ticks */
{
    ...
    invokes = 0;
    ticks = 0;
    while (CC == 3) {   /* Note: access exception if cryptography counter cannot be updated */
